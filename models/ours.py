...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        class_logits = class_logits.view(x.shape[0], -1)
...
def compute_class_loss(anchor_matches, class_pred_logits, shem_poolsize=20):
    ...
    if not 0 in pos_indices.size():
        ...
        pos_loss = F.binary_cross_entropy_with_logits(roi_logits_pos, torch.ones_like(roi_logits_pos).long(),
                                                      reduction='none')
        prob = F.sigmoid(roi_logits_pos)
        modulator = (1. - prob).pow(2)
    ...
    if not 0 in neg_indices.size():
        ...
        prob = F.sigmoid(roi_logits_neg)
        if shem_poolsize is None:
            neg_ix = torch.arange(neg_indices.cpu().data.numpy().size)
        else:
            negative_count = np.max((1, pos_indices.cpu().data.numpy().size))
            roi_probs_neg = torch.stack([1. - prob, prob], dim=1)
            neg_ix = mutils.shem(roi_probs_neg, negative_count, shem_poolsize)
        neg_loss = F.binary_cross_entropy_with_logits(roi_logits_neg[neg_ix],
                                                      torch.zeros_like(roi_logits_neg[neg_ix]).long(),
                                                      reduction='none')
        modulator = prob.pow(2)
        neg_loss = (neg_loss * modulator).mean()
    else:
...
def refine_detections(anchors, probs, deltas, regressions, batch_ixs, cf):
    ...
    flat_probs, flat_probs_order = probs.view(-1).sort(descending=True)
    keep_ix = flat_probs_order[:cf.pre_nms_limit]
    # reshape indices to 2D index array with shape like fg_probs.
    keep_arr = torch.cat(((keep_ix / probs.shape[1]).unsqueeze(1), (keep_ix % probs.shape[1]).unsqueeze(1)), 1)
...
class net(nn.Module):
    ...
    def build(self):
        ...
        self.fpn = backbone.FPN_pytorch_official(self.cf, conv, operate_stride1=self.cf.operate_stride1).cuda()
    ...
    def forward(self, img):
        ...
        flat_class_softmax = F.sigmoid(class_logits)
